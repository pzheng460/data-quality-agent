...
raw .tex
   ↓
preprocess.py（LaTeXML 之前）
   ├── 提取 algorithm 环境 → algorithm.py 解析为缩进伪代码
   ├── 提取 align*/gather* → 展开宏 + KaTeX 兼容 → $$...$$ 占位符
...
html_to_markdown（HTML → Markdown，核心转换）
   ...
   ├── 展开自定义宏（$\bx$ → $\mathbf{x}$）
   ├── KaTeX 兼容层（\mathbbm→\mathbb, \bm→\boldsymbol ...）
   ├── 清除 LaTeXML 拍平的 algorithmic 文本
   └── 恢复占位符（算法代码块、display math）
   ↓
最终输出 → 可直接渲染或用于预训练
```
