...
@click.option("--text-field", default="text", help="Field name containing text (default: text)")
@click.option("--data-type", type=click.Choice(["sft", "pretrain", "auto"]), default="auto",
              help="Data type — selects filter set (default: auto)")
@click.option("-w", "--workers", "parallelism", default=None, type=int,
              help="Parallel workers (default: auto)")
@click.option("--no-dedup", is_flag=True, default=False, help="Skip exact dedup")
def run(input_path: str, output_path: str, config_path: str | None,
        text_field: str, data_type: str, parallelism: int | None,
        no_dedup: bool):
    """Clean a dataset by filtering low-quality documents.

    Reads INPUT_PATH, applies quality filters in parallel, writes passing
    documents to OUTPUT_PATH. Supports JSONL and Parquet formats.

    \b
    Examples:
      dq run raw.jsonl cleaned.jsonl                        # local JSONL
      dq run raw.parquet cleaned.parquet                    # Parquet I/O
      dq run raw.jsonl cleaned.jsonl -c configs/sft.yaml    # custom config
      dq run raw.jsonl cleaned.jsonl -w 16                  # 16 workers
    """
    from dq.runner import run_cleaning

    # Load config — auto-select SFT config when applicable
    config = _load_config(config_path)
    if data_type == "sft" and config_path is None:
        sft_path = Path(__file__).parent.parent.parent / "configs" / "sft.yaml"
        if sft_path.exists():
            config = PipelineConfig.from_yaml(str(sft_path))

    console.print(f"[bold]dq run[/bold] — cleaning {input_path} → {output_path}")
    console.print(f"[dim]Config: {config_path or 'default'} | "
                  f"Filters: {sum(1 for f in config.filters if f.enabled)} enabled | "
                  f"Dedup: {'off' if no_dedup else 'on'} | "
                  f"Workers: {parallelism or 'auto'}[/dim]")
    ...
    console.print()
    console.print(f"[green bold]Done![/green bold]")
    console.print(f"  Input:      {stats['input_rows']:,} docs")
    console.print(f"  Output:     {stats['output_rows']:,} docs")
    console.print(f"  Dropped:    {stats['dropped']:,} ({stats['drop_rate']:.1%})")
    console.print(f"  Throughput: {stats['throughput']:,} docs/s")
