...
class TestLLMQualityScorer:
    """Tests for LLMQualityScorer."""

    def test_skip_when_no_transformers(self):
        """Scorer should pass all docs when transformers is not installed."""
        with patch("dq.model_filters.llm_scorer._check_transformers", return_value=False):
            from dq.model_filters.llm_scorer import LLMQualityScorer
            f = LLMQualityScorer.__new__(LLMQualityScorer)
            f.text_field = "text"
            f.params = {}
            f.model_name = "test-model"
            f.threshold = 3.0
    ...
    def test_filter_rejects_empty(self):
        """Scorer should reject empty text."""
        with patch("dq.model_filters.llm_scorer._check_transformers", return_value=True):
            from dq.model_filters.llm_scorer import LLMQualityScorer
            f = LLMQualityScorer.__new__(LLMQualityScorer)
            f.text_field = "text"
            f.params = {}
    ...
    def test_filter_with_mock_score(self):
        """Scorer should keep docs above threshold."""
        with patch("dq.model_filters.llm_scorer._check_transformers", return_value=True):
            from dq.model_filters.llm_scorer import LLMQualityScorer
            f = LLMQualityScorer.__new__(LLMQualityScorer)
            f.text_field = "text"
            ...
            with patch.object(f, "_score_text", return_value=4.2):
                keep, info = f.filter({"text": "educational content"})
                assert keep is True
                assert info["edu_score"] == 4.2

    def test_filter_drops_below_threshold(self):
        """Scorer should drop docs below threshold."""
        with patch("dq.model_filters.llm_scorer._check_transformers", return_value=True):
            from dq.model_filters.llm_scorer import LLMQualityScorer
            f = LLMQualityScorer.__new__(LLMQualityScorer)
            f.text_field = "text"
            ...
            with patch.object(f, "_score_text", return_value=1.5):
                keep, info = f.filter({"text": "low quality"})
                assert keep is False
                assert info["edu_score"] == 1.5

    def test_filter_registered(self):
        """LLMQualityScorer should be registered in the pipeline registry."""
        import dq.model_filters  # noqa: F401
        from dq.pipeline import get_filter_class
        cls = get_filter_class("llm_quality")
        assert cls.__name__ == "LLMQualityScorer"

    def test_score_documents_function(self):
        """score_documents should return a list of scores."""
        with patch("dq.model_filters.llm_scorer._check_transformers", return_value=True):
            from dq.model_filters.llm_scorer import LLMQualityScorer, score_documents

            with patch.object(LLMQualityScorer, "filter", return_value=(True, {"edu_score": 4.0})):
                scores = score_documents([{"text": "doc1"}, {"text": "doc2"}])
                assert len(scores) == 2
                assert all(s == 4.0 for s in scores)
