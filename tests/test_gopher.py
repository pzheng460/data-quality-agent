...
class TestGopherQualityFilter:
    ...
    def test_too_many_words(self):
        f = GopherQualityFilter(min_words=1, max_words=10, min_stopwords=0, min_lines_end_punct=0.0)
        text = " ".join(["word"] * 20) + "."
    ...
    def test_low_alpha_ratio(self):
        f = GopherQualityFilter(min_words=1, min_stopwords=0, min_lines_end_punct=0.0)
        text = "12345 67890. " * 50
        keep, info = f.filter({"text": text})
        assert not keep
    ...
    def test_configurable_text_field(self):
        f = GopherQualityFilter(text_field="content", min_words=2, min_stopwords=0, min_lines_end_punct=0.0)
    ...
    def test_too_few_stopwords(self):
        f = GopherQualityFilter(min_words=5, min_stopwords=2, min_lines_end_punct=0.0)
